...
| `/processes` | typed process-lifecycle repository routed through `ironclaw_filesystem` (append-only journal command rows plus capability process results/outputs) | process APIs | no | Consumer mount alias for `ironclaw_processes`; alias-relative under the per-invocation `MountView`. `/processes/journal/records` is the process-kernel authority; snapshots and lifecycle streams are projections over it. |

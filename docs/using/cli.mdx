...
| `ironclaw config set <key> <value>` | Write one value |
| `ironclaw profile list` | List the available boot profiles |

```bash
ironclaw config get llm.default.provider_id
ironclaw config set runner.poll_interval_ms 500
```

`config init` refuses to overwrite existing files unless you pass `--force`.

`config set` routes each key to the right destination — the configuration file, the
encrypted secret store, or the web token file — based on what the key is. See
...
| `ironclaw hooks list` | List configured hooks |
| `ironclaw channels list` | List configured channels |

Add `--verbose` to the `list` commands for extra detail.

<Note>
`channels list`, `hooks list`, and `logs` are registered but not implemented yet — they
report that and exit. Use the [web interface](/using/webui) for channel and extension

...
`extension install` takes an **extension id** from `extension search` — not a file path or
a URL. To install a package you built yourself, place it in one of the tool directories
above so `search` can find it, then install it by id.

To import a packaged extension archive directly, use **Extensions** in the

...
`config set` routes each key to the right destination based on what it is — the
configuration file, the encrypted secret store, or the web token file. You don't have to
know which.
...
| `hosted-single-tenant` | One tenant served over the network |
| `hosted-single-tenant-volume` | As above, with volume-backed storage |
...
`identity.tenant`, `default_agent`, and `default_project` are reserved. IronClaw rejects
...
and `max_concurrent_trigger_runs`. Each has an `IRONCLAW_REBORN_RUNNER_*` override.

...
<Step title="Use a served profile">

PostgreSQL pairs with a profile that expects it:
...
Use `hosted-single-tenant-volume` for volume-backed storage, or `production` for a
multi-user deployment. See [Configuration](/capabilities/configuration#profiles).

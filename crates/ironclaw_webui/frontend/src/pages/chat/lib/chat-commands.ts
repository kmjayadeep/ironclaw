...
// Returns the inventory descriptor the draft's first token names (by canonical
// name or alias), or null when the text is not a known slash command and
...
export function matchCommand(text, commands) {
  ...
  return (
    (commands || []).find(
      (command) =>
        command.name === first || (command.aliases || []).includes(first),
    ) || null
  );
}

// Inventory rows whose name/alias/title starts with the draft's command
// prefix — the derived composer menu.

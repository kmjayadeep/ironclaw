...
async fn gate_resolve_is_tenant_isolated_cross_tenant_grant_fails_closed() {
    ...
    // The ledger is keyed `(tenant, gate_ref)`; tenant A's binding drives this
    // gate, which is the `lk()` tenant. A missing row is strictly stronger than
    // a non-broadcast row, so treat NotFound as "never got that far".
    assert_ne!(
        ledger
            .state(&lk(&gate))
            .await

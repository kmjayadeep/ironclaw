...
use crate::{
    AdminConfigurationResolvedValues, AdminConfigurationService, AdminConfigurationServiceError,
};
    ...
    S: SecretStorePort + ?Sized,
{
    ...
    /// Resolve a complete credential set from one committed administrator
    /// configuration revision.
    ///
    /// This is the OAuth-client counterpart to [`Self::credential_handle_value`].
    /// Resolving the client ID and secret together prevents a concurrent
    /// rotation or a partial administrator record from mixing revisions.
    pub async fn credential_handle_values(
        &self,
        handles: &[SecretHandle],
    ) -> Result<Option<AdminConfigurationResolvedValues>, ExtensionAdminConfigurationResolverError>
    {
        for handle in handles {
            indexed_location(self.by_handle.get(handle), handle)?;
        }
        self.service

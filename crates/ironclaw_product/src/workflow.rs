...
/// Unknown-command reply. `InvalidRequest` (user-correctable) so downstream
/// feedback (the run-delivery observer, the WebUI endpoint) composes the
/// inventory-derived help — the rejection reason itself is redacted and never
/// reaches a surface.

...
async fn product_resolution(
    ...
    match outcome {
        ...
        RuntimeCapabilityOutcome::AttestedSigningRequired(_gate) => {
            ...
            // (the blocked run carries the binding on its durable record and
            // the attested resume verifies against it). A product/WebUI caller
            // has no such run, so a gate raised here could never be resolved.
            // Fail closed rather than hand back an unresumable gate.

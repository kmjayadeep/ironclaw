...
use crate::input::{
    ...
    RebornRuntimeProcessBinding, RebornStorageInput, RuntimeOwnerIdentity,
};
...
use crate::{
    RebornBuildError, RebornCompositionProfile, RebornFacadeReadiness, RebornHostBindings,
    RebornReadiness, RebornWorkerReadiness,
};
...
use ironclaw_host_runtime::{
    ...
    native_memory_first_party_package, register_outbound_delivery_first_party_handler,
};
use ironclaw_host_runtime::{
...
pub(crate) struct ChannelHostAssemblyWiring {
    ...
    pub(crate) run_delivery_events: Arc<ironclaw_product::RunDeliveryEventRouter>,
}
...
impl RebornRuntimeStores {
    ...
    pub(crate) fn start_channel_host_assembly(
        ...
        let admin_configuration_resolver = self.admin_configuration_resolver.clone();
        ...
        RebornStorageInput::LocalDev {
            ...
            build_local_storage_production_shaped(
                ...
                LocalStorageProductionInput {
                    ...
                    explicit_secret_master_key: secret_master_key,
...
async fn build_local_storage_production_shaped(
    ...
    // (`hosted-single-tenant-volume-sandboxed`) — gates the `/workspace`
    // per-user resolution below and the tenant concurrency ceiling / reaper
    // spawn in `build_backend_production`.
    ...
    // same host directory PER USER, not just for the boot owner.
    let workspace_root_mode = if is_sandboxed_profile {
        let owner_scope = match &context.local_runtime_identity {
...
struct RebornProductionBuildContext {
    ...
    /// constructed and injected into the exec transport, carried so
    /// `SandboxRuntimeBindings::build` reuses the SAME registry the transport
    /// writes into rather than a second, independently constructed one.
    /// `None` for every non-sandboxed profile and for the true production
    /// (libsql/Postgres) storage arms, which never set `runtime_process_binding`
    /// to `TenantSandbox`.
    sandbox_activity: Option<Arc<ironclaw_host_runtime::SandboxActivityRegistry>>,
...
async fn build_backend_production(
    ...
    // Captured before the move below (`apply_production_runtime_process_binding`
    // consumes `production_wiring.runtime_process_binding`); reused further down
    // to gate the sandbox runtime bindings without re-reading the moved field.
    let is_sandboxed_profile = matches!(
    ...
    let admin_configuration_resolver = Arc::new(ComposedExtensionAdminConfigurationResolver::new(
        Arc::clone(&admin_configuration),
        channel_egress_scope.clone(),
        available_manifests.clone(),
    ));
    ...
    let product_auth_services = Arc::new(
        product_auth_core.with_continuation_dispatcher(lifecycle_wrapped_auth_continuation),
    );
    ...
    let channel_host_wiring = {
        ...
        let channel_egress_credentials = Arc::new(
            crate::extension_host::channel_egress::AdminConfigurationEgressCredentials::new(
        ...
        let generic = crate::extension_host::generic_host::build_generic_extension_host(
            crate::extension_host::generic_host::GenericExtensionHostParams {
                ...
                admin_configuration_resolver: Some(Arc::clone(
                    &admin_configuration_resolver_for_generic,
                )),
        ...
        let channel_pairing_registry_built = {
            ...
            for descriptor in &account_setup_descriptors {
                ...
                let template_values = Arc::new(
                    crate::extension_host::channel_pairing::AdminConfigurationPairingTemplateValues::new(
                        Arc::clone(&admin_configuration_resolver_for_generic),
                        extension_id.clone(),
                    ),
                );
                ...
                let workflow_state_service =
                    ironclaw_product::ChannelWorkflowStateService::new(workflow_filesystem);
                let workflow_state = workflow_state_service
                    .build_for_extension(&extension_id, channel_egress_scope.clone())
                    .await
                    .map_err(|error| RebornBuildError::InvalidConfig {
                        reason: error.to_string(),
                    })?;
                ...
                let continuation = Arc::clone(&lifecycle_wrapped_product_continuation);
                ...
                let service = Arc::new(ChannelPairingService::new(
                    channel_egress_scope.tenant_id.clone(),
                    agent_id,
                    channel_egress_scope.project_id.clone(),
                    descriptor.clone(),
                    ChannelPairingServiceDependencies {
                        ...
                        template_values,
                        identity: Arc::new(
                            crate::extension_host::channel_pairing::ComposedChannelPairingIdentityStore::new(
                        ...
                        direct_targets: Arc::new(
                            crate::extension_host::channel_pairing::ComposedChannelPairingDirectTargetStore::new(
                                Arc::clone(&channel_dm_target_store),
                            ),
                        ),
                    },
                ));
        ...
        channel_pairing_registry_built
    ...
    // unlimited. `is_sandboxed_profile` also gates whether `build` does
    // anything at all with `sandbox_activity`/`sandbox_egress_proxy`; a no-op
    // `SandboxRuntimeBindings::none()`-shaped value comes back for every
    // non-sandboxed profile (and for the true production libsql/Postgres
    // storage arms, which never set `runtime_process_binding` to
    // `TenantSandbox`). `is_sandboxed_profile` was captured above, before
    // `production_wiring.runtime_process_binding` was moved into
    // `apply_production_runtime_process_binding`.
    let sandbox_runtime_bindings = crate::sandbox_composition::SandboxRuntimeBindings::build(
        crate::sandbox_composition::SandboxProfileBindingInputs {
            ...
            resource_governor: production_resource_governor.clone(),
            // Reuse the SAME registry `tenant_sandbox_process_binding`
            // injected into the exec transport (threaded here via
            // `RebornProductionBuildContext::sandbox_activity`), so the
            // reaper's idle reads observe the transport's writes.
            // Non-sandboxed profiles (and any caller that never wired one)
            // fall back to a fresh, never-touched registry —
            // `is_sandboxed_profile` gates whether `build` does anything
            // with it at all.
            ...
            // at), threaded here via
            // `RebornProductionBuildContext::sandbox_egress_proxy` so
            // `build` takes ownership of the SAME instance rather than

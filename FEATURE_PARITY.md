...
| GSuite WASM tools | ✅ | 🚧 | Reborn bundles operation-level Google Drive/Docs/Sheets/Slides WASM packages with host-mediated HTTP egress, product-auth scoped bearer injection, and manifest-declared Google OAuth setup metadata whose WebUI Admin values apply live ahead of boot-time environment fallbacks ([runtime resolver](crates/ironclaw_reborn_composition/src/product_auth/credentials/product_auth_providers.rs), [caller-level rotation test](tests/integration/admin_oauth_runtime/main.rs)); full live-recorded parity remains follow-up |
